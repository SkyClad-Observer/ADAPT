...
from coqpyt.coq.proof_file import ProofFile, ProofTerm
...
# TODO: fix this
def wrong_bullet(proof_file: ProofFile, proof_term: ProofTerm, groups, tactics: deque[Step]):
    _, bullet = groups
    tactics.popleft()
    tactics.appendleft(Step('\n' + bullet, bullet, None))
...
def no_more_goals(proof_file: ProofFile, proof_term: ProofTerm, groups, tactics: deque[Step]):
...
def next_goal(proof_file: ProofFile, proof_term: ProofTerm, groups, tactics: deque[Step]):
    bullet = groups[0]
    while tactics and not tactics[0].short_text == bullet:
        tactics.popleft()
